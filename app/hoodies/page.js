...
const page = async() => {
  ...
  if(!mongoose.connections[0].readyState){
        ...
        const product = await Product.find({category:"hoodies"})
        ...
        // const hoodiess = Object.values(hoodies)
...
{Object.values(hoodies).length===0 && <p>Out of stock</p>}

  {Object.values(hoodies).map((item)=>{return <Link key={item._id} href={`/product/${item.slug}`}legacyBehavior >
