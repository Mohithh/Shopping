...
export default  function Page() {
  ...
  const pincodebtn = async()=>{
    ...
    if(Object.keys(jsonpin).includes((pin))){
      toast.success('We can deliver to this pincode.', {
      ...
      toast.error(' we are unable to deliver to this pincode.', {
        position: "top-center",
        autoClose: 2000,
        hideProgressBar: false,
        closeOnClick: false,
...
const [finalproduct, setfinalproduct] = useState({})
  ...
  useEffect(() => {
    ...
    const alldata = async () => {
      ...
      const fetchdata = await fetch(`${process.env.NEXT_PUBLIC_HOST}/api/getproducts`);
...
const buynow = async () => {
    ...
    await clearcart(); 
    await addtocart(finalproduct.slug, 1, finalproduct.price, finalproduct.title, finalproduct.size, finalproduct.color);
    router.push('/checkout'); // Ensure the path is correct
  ...
  <div className="container px-5 py-10 mx-auto">
    ...
    {/* <img className='lg:w-1/3  h-[35vh] md:h-[auto] m-auto' src="https://m.media-amazon.com/images/I/613P0qrTDcL._AC_UL480_FMwebp_QL65_.jpg" alt="" /> */}
      ...
      <div className="lg:w-1/2 w-full lg:pl-10 lg:py-6 mt-6 lg:mt-0">
        ...
        <h1 className="text-gray-900 text-3xl title-font font-medium mb-1">{finalproduct.title}...{finalproduct.size}/{finalproduct.color}</h1>
      ...
      >
        ...
        <div className="">
          ...
          <span className="title-font font-medium md:text-2xl text-gray-900">₹{finalproduct.price}/-</span>

          <div className='flex flex-row mx-1 mt-5 items-center'>

         
          <button onClick={buynow} className="flex  mx-1 px-3 py-2 items-center text-white bg-pink-500 border-0  focus:outline-none hover:bg-pink-600 rounded">Buy Now</button>
          ...
          <button onClick={()=>{addtocart(finalproduct.slug,1,finalproduct.price,finalproduct.title,finalproduct.size,finalproduct.color),  
          ...
          theme: "light",
          transition: Bounce,
          });}} className="flex mx-1 px-2 py-2 text-white bg-pink-500 border-0 focus:outline-none hover:bg-pink-600 rounded">
  Add to cart 
</button> 
